"""Unit tests for channels/tcp_isn.py — roundtrip without network."""
...
class TestTcpIsnRoundtrip:
    ...
    def test_roundtrip_non_aligned(self) -> None:
        data = b"\x01\x02\x03\x04\x05"  # 5 bytes
        packets = self.channel.encode(data, dest_ip="127.0.0.1")
        result = self.channel.decode(packets)
        assert result == data
    ...
    def test_syn_flag(self) -> None:
        ...
        packets = self.channel.encode(data, dest_ip="127.0.0.1")
        for pkt in packets:
            assert pkt[TCP].flags & 0x02  # SYN flag
